...
### `<script setup>` - 더 간결한 Composition API
`<script setup>`은 Vue 3에서 Composition API를 더 쉽고 간결하게 사용하기 위한 문법이다. 이 키워드를 스크립트 태그에 추가하면, `setup()` 함수를 직접 선언하고 변수를 `return`할 필요 없이 코드를 바로 작성할 수 있다. 컴포넌트에 필요한 로직을 더 깔끔하게 관리할 수 있게 해주는 매우 유용한 기능이다.

---

### 컴포넌트 통신: `defineProps`와 `defineEmits`
`Vue`에서는 부모와 자식 컴포넌트가 명확한 규칙을 통해 데이터를 주고받을 수 있다.
- **`defineProps` (부모 → 자식):** 부모 컴포넌트가 자식 컴포넌트에게 데이터를 전달할 때 사용한다. (예. `src/components/ErrorModal.vue`에서는 부모로부터 `message`라는 문자열 데이터를 받아 화면에 표시)
- **`defineEmits` (자식 → 부모):** 자식 컴포넌트가 부모 컴포넌트에게 특정 이벤트가 발생했음을 알릴 때 사용한다. (예. `ErrorModal.vue`의 '확인' 버튼을 클릭하면 `'close'` 이벤트를 부모에게 보내 모달을 닫도록 요청.)
...
### `async def` - 비동기 API 엔드포인트
`FastAPI`의 큰 장점은 비동기 처리를 통해 높은 성능을 내는 것이다. `server/main.py`의 `async def get_weather_proxy(...)`와 같이 API 경로(엔드포인트)를 `async def`로 선언하면, 시간이 오래 걸리는 작업(예: 외부 API 호출)이 실행되는 동안 서버가 다른 요청을 동시에 처리할 수 있게 된다. 이는 서버의 응답성과 효율을 극대화하는 핵심 기능이다.   

---

### Pydantic `BaseModel` - 데이터 유효성 검사
`class RecommendationRequest(BaseModel):`와 같이 Pydantic의 `BaseModel`을 상속받아 클래스를 정의하면, FastAPI가 해당 엔드포인트로 들어오는 요청 본문(body)을 자동으로 검증한다. 만약 프론트엔드에서 `prompt` 필드를 빼먹거나 다른 타입으로 보내면, 서버는 "잘못된 요청"이라는 에러를 자동으로 응답한다. 이는 API의 신뢰성을 높이고 개발자가 직접 유효성 검사 코드를 작성하는 수고를 덜어준다.   

---

### 의존성 관리: `requirements.txt`와 `pip`
`server/venv` 가상 환경을 만든 후, 다른 개발자나 서버 환경에서 이 프로젝트를 똑같이 실행하려면 동일한 라이브러리들이 필요하다. `server/requirements.txt` 파일은 이 프로젝트가 필요로 하는 모든 Python 라이브러리와 그 버전의 목록으로 터미널에서 `pip install -r requirements.txt` 명령어를 실행하면, `pip`이 이 파일을 읽어 필요한 모든 라이브러리를 가상 환경에 자동으로 설치해준다.   

---

### `python-dotenv` - 민감 정보 관리
`python-dotenv` 라이브러리는 서버가 시작될 때 이 `.env` 파일을 읽어 환경 변수로 설정해주어, 코드에 직접 키를 노출하지 않고도 안전하게 사용할 수 있게 해준다.
